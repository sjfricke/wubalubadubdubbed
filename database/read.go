...
func ReadPhrase(db *sql.DB, phrases ...string) (PhraseEntry, int) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&entry.Phrase, &entry.File, &entry.Previous, &entry.Start, &entry.End, &entry.Next); err != nil {
			log.Println(err)
		}

		cRows, cErr := db.Query(`SELECT phrase, file, previousPhrase, startPhrase, endPhrase, nextPhrase
                                           FROM wubalubadubdub.words
		...
		defer rows.Close()
		for cRows.Next() {
			if cErr = cRows.Scan(&cEntry.Phrase, &cEntry.File, &cEntry.Previous, &cEntry.Start, &cEntry.End, &cEntry.Next); cErr != nil {
				log.Println(cErr)
			}
